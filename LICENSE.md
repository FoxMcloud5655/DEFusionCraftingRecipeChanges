...
**A jerk is anyone who attempts to or intends to claim partial or total ownership of the original or repackaged code and/or attempts to or intends to redistribute original or repackaged code without prior express written permission from the owner (FoxMcloud5655).
...
# Special Addition to the License

As long as this license file remains intact with no changes upon this mod's distribution, the user known as "Ryzen93900X" has full permission to do whatever they want with this code within reason. If they choose to make any kind of mod using a derivative of this code, it must use the original Don't Be A Jerk License by CoFH if the use of the code falls outside of the original scope of the license (i.e., if more than 50% of the code is used to create another work based on this code). More specifically, this means that they can publish this in its entirety or modified to include "additional recipes" for Draconic Evolution as compiled or source code in any method they choose to do so.

Copyright (c) 2023 FoxMcloud5655